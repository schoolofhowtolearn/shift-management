...
3. 管理者の「講習会シフト」タブ →「シフト確定・編集」で、日付×コマごとに「＋追加」からスタッフと生徒情報を割り当てて確定（同じコマに何人でも追加できるので、1コマに2人の講師を配置することも可能です。各コマには希望・可能人数も表示されます）
4. アルバイトの「確定シフト確認」タブ下部「講習会の確定シフト」で、自分に割り当てられた講習会シフトを確認可能
5. 不要になった講習会は「この講習会を削除」で、関連する希望・確定シフトごと削除できます

### 管理者による希望の直接編集

「希望一覧」タブ上部の「編集するスタッフ」で特定のスタッフを選ぶと、そのスタッフの希望（○/△）を管理者が直接編集できます。過去日付を含め、いつでもどのコマでも編集可能です（アルバイト本人の画面では過去日付は入力できませんが、管理者側には制限がありません）。「全員」を選ぶと元の集計表示（閲覧のみ）に戻ります。
